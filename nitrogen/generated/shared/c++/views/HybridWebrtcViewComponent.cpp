...
namespace margelo::nitro::webrtc::views {
  ...
  HybridWebrtcViewProps::HybridWebrtcViewProps(const react::PropsParserContext& context,
    ...
    resizeMode([&]() -> CachedProp<std::optional<std::string>> {
      try {
        const react::RawValue* rawValue = rawProps.at("resizeMode", nullptr, nullptr);
        if (rawValue == nullptr) return sourceProps.resizeMode;
        const auto& [runtime, value] = (std::pair<jsi::Runtime*, jsi::Value>)*rawValue;
        return CachedProp<std::optional<std::string>>::fromRawValue(*runtime, value, sourceProps.resizeMode);
      } catch (const std::exception& exc) {
        throw std::runtime_error(std::string("WebrtcView.resizeMode: ") + exc.what());
      }
    }()),
